...
class DeepSeek():
    ...
    def get_summary1(self, messages, roomid):
        ...
        summary_prompt = (
                ...
                "过程(50到200字左右)"
                "评价(50字以下)"
                "关键点总结(要求分点,类似keywords，每一点前使用'-')"
                "狲狲展望(100字左右，你对未来话题延申的猜测)"
                ...
                "6. 在总结的最后一段的下一行加上'DeepSeek----The Cheapest LLM :)'"
    ...
    def get_summary_of_partly(self, summaries):
        """根据微信群聊消息列表生成总结，由一名经验丰富的大学物理教授执行。"""
        summary_prompt = ( 
                "你是一名专业的聊天内容总结专家狲狲，你是一只兔狲，但是你很会总结话题，并且语言风格俏皮。现在需要你根据用户给出的多个之前生成的“分段总结”来制作一份每日总结报告。请按以下格式和要求进行总结：\n"
                "请你一步步思考，总结应覆盖较为重要的讨论内容，生成不多于10个的话题的总结（针对相似内容的话题进行合并，不要自行生成多余的内容）。总结包括以下内容："
                "话题名：(思考一个话题名并替换'话题名'，20字以内，带序号1️⃣2️⃣3️⃣，同时附带热度(热度根据成员讨论话题的数量决定），以🔥数量表示）\n"
                "👫参与者(不超过5个人，人名不重复)"
                "🕰️时间段(精确到分钟)"
                "讨论概述：50到200字左右，描述这段时间内的整体讨论内容"
                "亮点评论：50字以下，对讨论内容的整体评价"
                "关键点总结(要求分点,类似keywords，每一点前使用'-')"
                "狲狲展望(100字左右，你对未来话题延申的猜测)"
                "emoji叙事：给整个总结后面增加三个有关话题的emoji表情符号"
                "分割线： ---------------------    "   
                "另外有以下要求："
                "1. 每个话题分段并空一行"
                "2. 使用中文冒号"
                ...
                "4. 找出内容较为相似的话题进行合并"
                "5. 开始给出本群讨论风格的整体评价，例如：本群讨论很活跃哦！、本群无关话题太多了好水鸭、此群好像话题不集中呢、本群的话题有些无聊呢等等诸如此类，最好加点俏皮语气词（要求以一个话题专家狲狲的口吻回答）"
                "6. 使用提供的“分段总结”内容，确保总结内容与其大致相似，不要生成幻觉内容，严格依据提供的内容进行总结。"
                "7. 请从每段时间的总结中选取1至4个🔥数量较多或者比较重要的话题进行总结，一共不超过10个话题，可以将相似内容的话题进行合并。"
                "8. 在总结的最后一段的下一行加上'DeepSeek----The Cheapest LLM :)'"
        )
        summaries_str = "接下来是一段时间的总结".join(summaries) # 这会使每一时间段的话题总结之间增加间隔符“接下来是...”
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
