...
class DeepSeek():
    ...
    def get_summary_of_partly(self, summaries, roomid):
        ...
        summary_prompt = (
                ...
                "2. 使用中文冒号"
                "3. 无需大标题"
                "4. 根据话题的🔥数量判断各种话题的重要程度，可以优先考虑对重要话题进行总结"
                "5. 找出内容较为相似的话题进行合并"
                "6. 开始给出本群讨论风格的整体评价，例如：本群讨论很活跃哦！、本群无关话题太多了好水鸭、此群好像话题不集中呢、本群的话题有些无聊呢等等诸如此类，最好加点俏皮语气词（要求以一个话题专家狲狲的口吻回答）"
                "7. 使用提供的“分段总结”内容，确保总结内容与其大致相似，不要生成幻觉内容，严格依据提供的内容进行总结。"
                "8. 请从每段时间的总结中选取1至4个比较重要的话题进行总结，一共不超过10个话题，可以将相似内容的话题进行合并。"
        )
